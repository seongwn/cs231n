...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        sample_mean = np.sum(x,axis=0) / x.shape[0]
        xm = x - sample_mean
        sample_var = np.sum(xm**2,axis=0) / x.shape[0]
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #(sample_var, normalized_x, x-sample_mean, gamma, eps)
    (var, x_hat, xm, gamma, eps) = cache
    N, D = np.shape(x_hat)
    dbeta = np.sum(dout, axis=0) #(D,)
    
    dgamma = np.sum(np.multiply(dout,x_hat),axis=0) #(D,)
    
    dx_hat = np.multiply(dout,gamma) #(N,D)
    
    sqrt_var = np.sqrt(var + eps)
    i_var = 1 / sqrt_var
    dxm1 = np.multiply(dx_hat,i_var) #(N,D)
    
    di_var =  np.sum(np.multiply(dx_hat,xm),axis=0) #(D,)
    dsqrt_var = np.multiply(di_var,(-1)/(sqrt_var**2)) #(D,)
    dvar = dsqrt_var * (0.5) * i_var #(D,)
    dxmxm = np.ones((N,D)) * dvar / N #(N,D)
    dxm2 = dxmxm * 2 * xm #(N,D)
    dx1 = dxm1 + dxm2 #(N,D)
    dmb = -1 * np.sum(dxm1+dxm2,axis=0) #(D,)
    dx2 = np.multiply(np.ones((N,D)), dmb) / N #(N,D)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat*xm,axis=0) * (-0.5) / ((var + eps) * np.sqrt(var + eps))
    dmu = (-1)*np.sum(dx_hat/np.sqrt(var+eps),axis=0) + (dvar) * np.sum(-2 * xm,axis=0) / x_hat.shape[0]
    dx = dx_hat * 1. / np.sqrt(var+eps) + dvar * 2 * xm / x_hat.shape[0] + dmu / x_hat.shape[0]
    
    #dx = (dout * gamma) * 1. / np.sqrt(var+eps) + (np.sum((dout * gamma)*xm,axis=0) * (-0.5) / ((var + eps) * np.sqrt(var + eps))) * 2 * xm / x_hat.shape[0] + ((-1)*np.sum((dout * gamma)/np.sqrt(var+eps),axis=0) + (np.sum((dout * gamma)*xm,axis=0) * (-0.5) / ((var + eps) * np.sqrt(var + eps))) * np.sum(-2 * xm,axis=0) / x_hat.shape[0]) / x_hat.shape[0]
...
def layernorm_backward(dout, cache):
    ...
    di_var =  np.sum(np.multiply(dx_hat,xm)) #(1,)
    dsqrt_var = np.multiply(di_var,(-1)/(sqrt_var**2)) #(1,)
    dvar = dsqrt_var * (0.5) * i_var #(1,)
    dxmxm = np.ones((N,D)) * dvar / H #(N,D)
    dxm2 = dxmxm * 2 * xm #(N,D)
    dx1 = dxm1 + dxm2 #(N,D)
    dmb = -1 * np.sum(dxm1+dxm2) #(1,)
    dx2 = np.multiply(np.ones((N,D)), dmb) / H #(N,D)
